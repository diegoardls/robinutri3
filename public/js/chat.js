// chat.js - VERSIÓN COMPLETA CON ELIMINAR Y RENOMBRAR
...
async function enviarMensajeIA(mensaje, chatId, perfilId) {
    try {
        ...
        if (data.success) {
            // ⭐⭐ ACTUALIZAR chat_id SI ES UN NUEVO CHAT
...
// Función para cargar historial cuando se selecciona un chat
...
function cargarHistorialDesdeBD(perfilId = null) {
    ...
    // ⭐⭐ ACTUALIZAR PERFIL ACTIVO SI SE PROVEE
    if (perfilId) {
    ...
    fetch(url)
        ...
        .then(data => {
            ...
            // ⭐⭐ VERIFICAR QUE LOS CHATS PERTENEZCAN AL PERFIL CORRECTO
...
// ⭐⭐ CREAR ELEMENTO CHAT INDIVIDUAL
...
// En la función inicializarBotonesAccion, agrega:
...
// ⭐⭐ FUNCIONES PARA RENOMBRAR CHATS
...
function inicializarSistemaChat() {
    ...
    // ⭐⭐ OBTENER PERFIL ACTIVO DE LA URL
    ...
    // ⭐⭐ CARGAR CHATS CON FILTRO ESTRICTO
...
// ⭐⭐ INICIALIZAR SIDEBAR DE PERFILES
...
// ⭐⭐ FUNCIÓN GLOBAL PARA CAMBIAR PERFIL (desde HTML)
...
function actualizarInterfazPerfil() {
    ...
    // Actualizar botón "Nuevo chat" con el perfil activo
...
// ⭐⭐ MANEJAR CAMBIOS EN LA URL (navegación hacia atrás/adelante)
...
function verificarEstadoSistema() {
    ...
    // Verificar si los perfiles están marcados como activos
...
async function enviarMensajeIA(mensaje, chatId, perfilId) {
    try {
        ...
        // ⭐⭐ VOLVER A USAR EL ENDPOINT PRINCIPAL
        // ⭐⭐ USA ESTE ENDPOINT QUE SÍ FUNCIONA
        const response = await fetch('/robinutri/api_final.php', {
            method: 'POST',
            body: formData
        });
...
// Función para mostrar indicador de typing - CORREGIDA
function mostrarLoadingIndicator() {
    ...
    const chatArea = document.getElementById('chat-area');
    if (chatArea) {
        // ⭐⭐ CORRECCIÓN: Usar innerHTML en lugar de appendChild
...
// Función para obtener el ID del perfil activo
